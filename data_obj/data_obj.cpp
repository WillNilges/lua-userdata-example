...
static int dataobj_new(lua_State* L)
{
	...
	*reinterpret_cast<DataObj**>(lua_newuserdata(L, sizeof(DataObj*))) = new DataObj(length);
	luaL_setmetatable(L, LUA_DATAOBJ);
	return 1;
}
...
{
	(*reinterpret_cast<DataObj**>(luaL_checkudata(L, 1, LUA_DATAOBJ)))->set(luaL_checkinteger(L, 2), luaL_checkinteger(L, 3));
	return 0;
}

static int dataobj_get(lua_State* L)
{
	lua_pushnumber(L, (*reinterpret_cast<DataObj**>(luaL_checkudata(L, 1, LUA_DATAOBJ)))->get(luaL_checkinteger(L, 2)));
	return 1;
}
 
// Register DataObj to Lua
static void register_dataobj(lua_State* L){
	lua_register(L, LUA_DATAOBJ, dataobj_new);
	luaL_newmetatable(L, LUA_DATAOBJ);
